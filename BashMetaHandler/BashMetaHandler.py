...
def check(BashHandler,search_string):
	...
	for iK in range(2):
		...
		if len(BashHandler.history) -1-iK >= 0:
			i = BashHandler.history[len(BashHandler.history)-1-iK].find(search_string)
			...
			if i != -1:
				...
				print(f" {search_string} found in {BashHandler.history[len(BashHandler.history)-1-iK]}")
				
				return True
			print(f" {search_string} not found in {BashHandler.history[len(BashHandler.history)-iK-1]}")
	return False
...
def equal(BashHandler,args,argf = []):
	if argf == []:
		...
		if split_arg[1].find("(") != 0 and (split_arg[1][0] != '"' and split_arg[1][len(split_arg[1])-1] != '"'):
			try:
		...
		if split_arg[0].find("(") != 0 and (split_arg[0][0] != '"' and split_arg[0][len(split_arg[0])-1] != '"'):
			try:
...
def expect_check(BashHandler,search_string_and_timeout):
	...
	while time_dif.seconds+1e-6*time_dif.microseconds < timeout:
		...
		if nwLines != None: print(f"nwLines: {nwLines}")
...
class metaBashHandler:
	def __init__(self,filename='',variableDict={},functionDict={"println":println,"print":println,'check':check,'wait':wait,'expect':expect_check,'input':get_input,'not':do_not,'equal':equal}):
			...
			try:
				...
				if iLast != -1:
					...
					while iB >= 0:
						...
						if (iOpen < iClose or iClose == -1) and (iOpen < iQuotes or  iQuotes == -1) and (iOpen < iComma or  iComma == -1) and iOpen != -1:
							iLast = iOpen
							if iB == 0:
								iBegin = iLast +1
							iB = iB +1
							continue
						elif (iClose < iOpen or iOpen == -1) and (iClose < iQuotes or iQuotes == -1) and (iClose < iComma or  iComma == -1) and iClose != -1: 
							iB = iB - 1
							iLast = iClose
							if iB == 0:
								...
								if iComma_arg == -1:
									...
									iLast = iComma_arg+1
							continue
						elif (iQuotes < iOpen or iOpen == -1) and (iQuotes < iClose or iClose == -1) and (iQuotes < iComma or  iComma == -1) and iQuotes != -1:
							iQuotes = func_arg.find("\"",iQuotes+1)
							iLast = iQuotes
						elif (iComma < iClose or iClose == -1) and (iComma < iOpen or iOpen == -1) and (iComma < iQuotes or iQuotes == -1) and iComma != -1:
					...
					for iInFunc in range(len(func_names)):
						if func_names[iInFunc] != None:
							res = self.callFunc(func_names[iInFunc]+"("+func_args[iInFunc]+")",iLine)
						else:
				...
				print(f"({iLine+1}) function {func_name}({func_arg}) not executed properly")	
				#raise
				...
				if realDepth == iDepth:
					...
					if lastCall == "if" or lastCall == "elif":
						...
						while l_spl[0] == "else" or l_spl[0] == "elif":
							...
							l_spl = self.lines[iL][realDepth:len(self.lines[iL])].split(" ")
						iLine = iL
						iDepth = realDepth
						continue
					elif lastCall == "while":
				...
				if l_spl[0] in conditional_terms:
					...
					while l_spl[0] in conditional_terms:
						if l_spl[0] == "if" or l_spl[0] == "elif":
							if self.callFunc(line[len(l_spl[0])+1:len(line)],iLine):
								iL = iL + 1
								break
							else:
								iL = iL + 1
								...
								l_spl = self.lines[iL][iDepth:len(self.lines[iL])].split(" ")
								#print(l_spl)
						elif l_spl[0] == "while":
							if self.callFunc(line[len(l_spl[0])+1:len(line)],iLine):
								last_while = iL
					...
					if go_into:
						funcCallList.append(l_spl[0].replace("\n","").replace("\r",""))
						iDepth = iDepth +1
					iLine = iL
