...
`mypyc/test-data/`, and you can run all mypyc tests via `pytest -q mypyc`. If you don't make changes to code under `mypy/`, it's not
...
`mypyc.test.test_run`. The code to compile comes after `[case test<name>]`. The code gets saved into the file `native.py`, and it
