...
from git.types import HSH_TD
from git.types import Files_TD
from git.types import Has_id_attribute
from git.types import Literal
from git.types import PathLike
from git.types import Protocol
from git.types import SupportsIndex
from git.types import Total_TD
from git.types import runtime_checkable
...
def py_where(program: str, path: Optional[PathLike] = None) -> List[str]:
    ...
    for folder in str(path).split(os.pathsep):
        ...
        if folder:
            ...
            for f in [exe_path] + [f"{exe_path}{e}" for e in winprog_exts]:
                if is_exec(f):
                    progs.append(f)
    return progs
...
def _cygexpath(drive: Optional[str], path: str) -> str:
    if osp.isabs(path) and not drive:
        ...
        if osp.isabs(p):
            ...
            p = f"/proc/cygdrive/{drive.lower()}/{p}"
...
class Actor:
    ...
    def __repr__(self) -> str:
        return f'<git.Actor "{self.name} <{self.email}>">'
