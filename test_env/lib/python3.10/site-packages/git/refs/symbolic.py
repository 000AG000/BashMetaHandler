...
class SymbolicReference:
    ...
    def __repr__(self) -> str:
        return f'<git.{self.__class__.__name__} "{self.path}">'
    ...
    def _get_reference(self) -> "SymbolicReference":
        ...
        if target_ref_path is None:
            raise TypeError(
                "{} is a detached symbolic reference as it points to {!r}".format(
                    self, sha
                )
            )
    ...
    def to_full_path(cls, path: Union[PathLike, "SymbolicReference"]) -> PathLike:
        ...
        if not str(path).startswith(cls._common_path_default + "/"):
            full_ref_path = f"{cls._common_path_default}/{path}"
        return full_ref_path
    ...
    def from_path(
        ...
        from . import RemoteReference
        from . import TagReference
