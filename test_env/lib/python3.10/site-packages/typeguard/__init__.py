...
def check_callable(argname: str, value, expected_type, memo: _TypeCheckMemo) -> None:
    if not callable(value):
        raise TypeError(f"{argname} must be a callable")
        ...
        if hasattr(expected_type, "__args__") and expected_type.__args__ not in (
            ...
            if key_type is not Any or value_type is not Any:
                for k, v in value.items():
                    check_type(f"keys of {argname}", k, key_type, memo)
                    check_type(f"{argname}[{k!r}]", v, value_type, memo)
...
def check_typed_dict(argname: str, value, expected_type, memo: _TypeCheckMemo) -> None:
    ...
    if extra_keys:
        ...
        raise TypeError(f"extra key(s) ({keys_formatted}) in {argname}")

    missing_keys = required_keys - existing_keys
    if missing_keys:
        keys_formatted = ", ".join(f'"{key}"' for key in sorted(missing_keys))
        raise TypeError(f"required key(s) ({keys_formatted}) missing from {argname}")

    for key, argtype in get_type_hints(expected_type).items():
        argvalue = value.get(key, _missing)
        if argvalue is not _missing:
            check_type(f'dict item "{key}" for {argname}', argvalue, argtype, memo)
        ...
        if hasattr(expected_type, "__args__") and expected_type.__args__ not in (
            ...
            if value_type is not Any:
                for i, v in enumerate(value):
                    check_type(f"{argname}[{i}]", v, value_type, memo)
...
def check_set(argname: str, value, expected_type, memo: _TypeCheckMemo) -> None:
    ...
    if expected_type is not set:
        if hasattr(expected_type, "__args__") and expected_type.__args__ not in (
            ...
            if value_type is not Any:
                for v in value:
                    check_type(f"elements of {argname}", v, value_type, memo)
...
def check_tuple(argname: str, value, expected_type, memo: _TypeCheckMemo) -> None:
    ...
    if is_named_tuple:
        ...
        for name, field_type in field_types.items():
            check_type(f"{argname}.{name}", getattr(value, name), field_type, memo)
    ...
    if use_ellipsis:
        ...
        for i, element in enumerate(value):
            check_type(f"{argname}[{i}]", element, element_type, memo)
    elif tuple_params == ((),):
        if value != ():
            raise TypeError(f"{argname} is not an empty tuple but one was expected")
    else:
...
class TypeCheckedGenerator:
    ...
    def send(self, obj):
        if self.__initialized:
            check_type(
                "value sent to generator", obj, self.__send_type, memo=self.__memo
            )
        else:
        ...
        check_type(
            "value yielded from generator", value, self.__yield_type, memo=self.__memo
        )
        return value
        ...
        if event == "call":
            ...
            if memo is not None:
                try:
                    if memo.is_generator:
                        check_type(
                            "yielded value", arg, memo.type_hints["return"], memo
                        )
                    else:
