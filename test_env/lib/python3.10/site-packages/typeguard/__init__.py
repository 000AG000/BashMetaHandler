...
def check_typed_dict(argname: str, value, expected_type, memo: _TypeCheckMemo) -> None:
    ...
    if missing_keys:
        ...
        raise TypeError(
            f"required key(s) ({keys_formatted}) missing from {argname}"
        )
    ...
    for key, argtype in get_type_hints(expected_type).items():
        ...
        if argvalue is not _missing:
            check_type(
                f'dict item "{key}" for {argname}', argvalue, argtype, memo
            )
...
def check_tuple(argname: str, value, expected_type, memo: _TypeCheckMemo) -> None:
    ...
    if is_named_tuple:
        ...
        for name, field_type in field_types.items():
            check_type(
                f"{argname}.{name}", getattr(value, name), field_type, memo
            )
    ...
    if use_ellipsis:
        ...
        if value != ():
            raise TypeError(
                f"{argname} is not an empty tuple but one was expected"
            )
    else:
