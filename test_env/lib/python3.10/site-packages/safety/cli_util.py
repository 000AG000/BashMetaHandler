...
from safety.auth.constants import CLI_AUTH
from safety.auth.constants import MSG_NON_AUTHENTICATED
from safety.auth.models import Auth
from safety.constants import MSG_NO_AUTHD_CICD_PROD_STG
from safety.constants import MSG_NO_AUTHD_CICD_PROD_STG_ORG
from safety.constants import MSG_NO_AUTHD_DEV_STG
from safety.constants import MSG_NO_AUTHD_DEV_STG_ORG_PROMPT
from safety.constants import MSG_NO_AUTHD_DEV_STG_PROMPT
from safety.constants import MSG_NO_AUTHD_NOTE_CICD_PROD_STG_TPL
from safety.constants import MSG_NO_VERIFIED_EMAIL_TPL
...
from .errors import SafetyException
...
def pretty_format_help(
    ...
    from typer.rich_utils import OPTIONS_PANEL_TITLE
    from typer.rich_utils import STYLE_USAGE_COMMAND
...
def print_main_command_panels(
    *, name: str, commands: List[click.Command], markup_mode: MarkupMode, console
) -> None:
    """
    ...
    from rich.table import Table
    from rich.text import Text
    from typer.rich_utils import ALIGN_COMMANDS_PANEL
    from typer.rich_utils import STYLE_COMMANDS_PANEL_BORDER
    from typer.rich_utils import STYLE_COMMANDS_TABLE_BORDER_STYLE
    from typer.rich_utils import STYLE_COMMANDS_TABLE_BOX
    from typer.rich_utils import STYLE_COMMANDS_TABLE_LEADING
    from typer.rich_utils import STYLE_COMMANDS_TABLE_PAD_EDGE
    from typer.rich_utils import STYLE_COMMANDS_TABLE_PADDING
    from typer.rich_utils import STYLE_COMMANDS_TABLE_ROW_STYLES
    from typer.rich_utils import STYLE_COMMANDS_TABLE_SHOW_LINES
    ...
    with typer_console.use_theme(Theme(styles=CONSOLE_HELP_THEME)) as theme_context:
        ...
        if isinstance(obj, click.MultiCommand):
            ...
            for command_name in obj.list_commands(ctx):
                ...
                if command and not command.hidden:
                    panel_name = (
                        UTILITY_COMMANDS_PANEL_TITLE
                        if command.utility_command
                        else COMMANDS_PANEL_TITLE
                    )
                    panel_to_commands[panel_name].append(command)
...
class SafetyCLILegacyGroup(UtilityCommandMixin, click.Group):
    ...
    def invoke(self, ctx: click.Context) -> None:
        ...
        PROXY_HOST_OPTIONS = {"--proxy-host", "-ph"}
        if (
