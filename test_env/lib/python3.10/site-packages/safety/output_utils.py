...
from safety.constants import RED
from safety.constants import YELLOW
...
from safety.util import build_git_data
    ...
    if cve:
        ...
        if cve.cvssv3 and "base_severity" in cve.cvssv3.keys():
            ...
            cve_lines = [
                {
                    "words": [
                        {"style": {"bold": True}, "value": f"{cve.name} is "},
                        {
    ...
    to_print = [
        {
            "words": [
                ...
                {
                    "value": " version {} found using license ".format(
                        license["version"]
                    )
                },
...
def get_final_brief(
    ...
    remediation_text = (
        f" {rem_count} remediation{' was' if rem_count == 1 else 's were'} "
        f"recommended."
        if is_using_api_key()
        else ""
    )
...
def get_final_brief_license(
    licenses: List[str], kwargs: Optional[Dict[str, Any]] = None
) -> str:
    """
    ...
    if licenses:
        licenses_text = (
            "The following software licenses were present in your system: {}".format(
                ", ".join(licenses)
            )
        )

    return format_long_text(
        f"{licenses_text}", start_line_decorator=" ", **kwargs
    )
    ...
    for line in lines:
        ...
        if line == "":
            ...
            formatted_lines.append(
                f"{start_line_decorator}{empty_line}{end_line_decorator}"
            )
...
def build_scanned_count_sentence(packages: List[Package]) -> List[Dict[str, Any]]:
    ...
    if len(packages) >= 1:
        scanned_count = "Found and scanned {} {}".format(
...
def get_report_brief_info(as_dict: bool = False, report_type: int = 1, **kwargs: Any):
    ...
    if policy_file and policy_file.get("filename", False):
        safety_policy_used = [
            ...
            {"style": True, "value": " {}".format(policy_file.get("filename", "-"))},
        ]
