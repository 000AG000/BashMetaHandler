...
from safety.auth.constants import CLI_AUTH_SUCCESS
from safety.auth.constants import CLI_LOGOUT_SUCCESS
from safety.auth.constants import HOST
...
def process_browser_callback(uri: str, **kwargs: Any) -> Any:
    ...
    try:
        ...
        if not headless:
            ...
            server_thread.start()
            message = (
                f"If the browser does not automatically open in 5 seconds, "
        ...
        if e.errno == socket.errno.EADDRINUSE:
            reason = (
                f"The port {HOST}:{PORT} is currently being used by another"
