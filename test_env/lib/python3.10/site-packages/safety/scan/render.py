...
from safety_schemas.models import IgnoreCodes
from safety_schemas.models import PolicyFileModel
...
def print_brief(
    ...
    console.print(
        f"Tested [number]{dependencies_count}[/number] {pluralize('dependency', dependencies_count)} for known security "
...
def print_ignore_details(
    ...
    if is_detailed_output:
        ...
        if len(ignored) > 0:
            console.print(
                f"([number]{len(ignored)}[/number] {pluralize('vulnerability', len(ignored))} {pluralize('was', len(ignored))} ignored due to "
                "project policy)"
            )
...
def prompt_link_project(console: Console, prj_name: str, prj_admin_email: str) -> bool:
    ...
    """
    console.print(
        "[bold]Safety found an existing project with this name in your organization:[/bold]"
    )

    for detail in (
        f"[bold]Project name:[/bold] {prj_name}",
...
def create_pkg_ext_ref(*, package: PythonDependency, version: Optional[str]) -> Any:
    ...
    """
    from spdx_tools.spdx.model import ExternalPackageRef
