...
from safety.scan.constants import CLI_SCAN_COMMAND_HELP
from safety.scan.constants import CLI_SYSTEM_SCAN_COMMAND_HELP
from safety.scan.constants import CMD_PROJECT_NAME
from safety.scan.constants import CMD_SYSTEM_NAME
from safety.scan.constants import DEFAULT_EPILOG
from safety.scan.constants import DEFAULT_SPINNER
from safety.scan.constants import SCAN_APPLY_FIXES
from safety.scan.constants import SCAN_DETAILED_OUTPUT
from safety.scan.constants import SCAN_OUTPUT_HELP
from safety.scan.constants import SCAN_POLICY_FILE_HELP
from safety.scan.constants import SCAN_SAVE_AS_HELP
from safety.scan.constants import SCAN_TARGET_HELP
from safety.scan.constants import SYSTEM_SCAN_OUTPUT_HELP
from safety.scan.constants import SYSTEM_SCAN_POLICY_FILE_HELP
from safety.scan.constants import SYSTEM_SCAN_SAVE_AS_HELP
from safety.scan.constants import SYSTEM_SCAN_TARGET_HELP
from safety.scan.decorators import inject_metadata
from safety.scan.decorators import scan_project_command_init
from safety.scan.decorators import scan_system_command_init
from safety.scan.finder import FileFinder
from safety.scan.finder.file_finder import should_exclude
from safety.scan.finder.handlers import ECOSYSTEM_HANDLER_MAPPING
from safety.scan.finder.handlers import FileHandler
from safety.scan.main import load_policy_file
from safety.scan.main import load_unverified_project_from_config
from safety.scan.main import process_files
from safety.scan.main import save_report_as
from safety.scan.models import ScanExport
from safety.scan.models import ScanOutput
from safety.scan.models import SystemScanExport
from safety.scan.models import SystemScanOutput
from safety.scan.render import print_brief
from safety.scan.render import print_detected_ecosystems_section
from safety.scan.render import print_fixes_section
from safety.scan.render import print_ignore_details
from safety.scan.render import render_scan_html
from safety.scan.render import render_scan_spdx
from safety.scan.render import render_to_console
from safety.scan.util import Stage
from safety.scan.validators import output_callback
from safety.scan.validators import save_as_callback
from safety.util import pluralize
from safety_schemas.models import Ecosystem
from safety_schemas.models import FileModel
from safety_schemas.models import FileType
from safety_schemas.models import ProjectModel
from safety_schemas.models import ReportModel
from safety_schemas.models import ScanType
from safety_schemas.models import SecurityUpdates
from safety_schemas.models import Vulnerability
from safety_schemas.models import VulnerabilitySeverityLabels
from typing_extensions import Annotated

from ..cli_util import SafetyCLICommand
from ..cli_util import SafetyCLISubGroup
from ..cli_util import handle_cmd_exception
        ...
        if output is ScanOutput.SCREEN:
            ...
            if obj.platform_enabled and report_url:
                if report.metadata.scan_type is ScanType.scan:
                    ...
                    lines.append("Project dashboard: " f"[link]{project_url}[/link]")
...
def scan(
    ...
    with console.status(wait_msg, spinner=DEFAULT_SPINNER) as status:
        for path, analyzed_file in process_files(paths=file_paths, config=config):
            ...
            if detailed_output:
                vulns_ignored = (
                    analyzed_file.dependency_results.ignored_vulns_data.values()
                )
                ignored_vulns_data = itertools.chain(vulns_ignored, ignored_vulns_data)
            ...
            if any(affected_specifications):
                ...
                for spec in affected_specifications:
                    ...
                    console.print(
                        Padding(f"{msg}]", (0, 0, 0, 1)), emoji=True, overflow="crop"
                    )
                    ...
                    console.print(
                        Padding(
                            f"Learn more: [link]{spec.remediation.more_info_url}[/link]",
                            (0, 0, 0, 1),
                        ),
                        emoji=True,
                    )
                ...
                console.print()
                console.print(
                    f":white_check_mark: [file_title]{path.relative_to(target)}: No issues found.[/file_title]",
                    emoji=True,
                )
    ...
    console.print()
    print_brief(console, ctx.obj.project, count, affected_count, fixes_count)
    print_ignore_details(
        console,
        ctx.obj.project,
        ignored,
    ...
    if output is not ScanOutput.NONE:
        if detailed_output:
            if exit_code > 0:
                console.print(
                    f":stop_sign: Scan-failing vulnerabilities were found, returning non-zero exit code: {exit_code}"
                )
            else:
                console.print(
