    ...
    def _traverse(
        ...
        if include_self and descend and not siblings:
            if condition is None:
                yield from self._all_traverse()
                return
            elif isinstance(condition, type):
                yield from self._fast_traverse(condition)
                return
...
class Text(Node, reprunicode):
    ...
    def shortrepr(self, maxlen=18):
        ...
        if len(data) > maxlen:
            data = data[: maxlen - 4] + " ..."
        return "<{}: {!r}>".format(self.tagname, reprunicode(data))
...
class Element(Node):
    ...
    def emptytag(self):
        return "<%s/>" % " ".join(
            [self.tagname] + ['{}="{}"'.format(n, v) for n, v in self.attlist()]
        )
    ...
    def replace_self(self, new):
        ...
        if isinstance(update, Element):
            ...
            for att in self.basic_attributes:
                assert not self[att], 'Losing "{}" attribute: {}'.format(att, self[att])
...
class pending(Special, Invisible, Element):
    ...
    def pformat(self, indent="    ", level=0):
        ...
        return Element.pformat(self, indent, level) + "".join(
            [("    {}{}\n".format(indent * level, line)) for line in internals]
        )
