...
class Text(Node, reprunicode):
    ...
    def shortrepr(self, maxlen=18):
        ...
        if len(data) > maxlen:
            data = data[: maxlen - 4] + " ..."
        return f"<{self.tagname}: {reprunicode(data)!r}>"
...
class Element(Node):
    ...
    def emptytag(self):
        return "<%s/>" % " ".join(
            [self.tagname] + [f'{n}="{v}"' for n, v in self.attlist()]
        )
    ...
    def replace_self(self, new):
        ...
        if isinstance(update, Element):
            ...
            for att in self.basic_attributes:
                assert not self[att], f'Losing "{att}" attribute: {self[att]}'
