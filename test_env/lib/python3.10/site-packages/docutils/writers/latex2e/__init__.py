...
class CharMaps:
    ...
    textcomp = {
        ...
        0x2117: "\\textcircledP{}",  # SOUND RECORDING COYRIGHT
...
class Table:
    ...
    def get_colspecs(self, node):
        ...
        for node in self._col_specs:
            ...
            latex_table_spec += "{}p{{{:.3f}\\DUtablewidth}}".format(bar, colwidth + 0.005)
        return latex_table_spec + bar
...
class LaTeXTranslator(nodes.NodeVisitor):
        ...
        if not self.is_xetex:
            ...
            for ch in text:
                ...
                # preamble-definitions for unsupported Unicode characters
                    ...
                    self.requirements[
                        "_inputenc" + str(cp)
                    ] = "\\DeclareUnicodeCharacter{{{:04X}}}{{{}}}".format(
                        cp,
    ...
    def visit_enumerated_list(self, node):
        ...
        if enumeration_level <= 4:
            ...
            if (prefix, enumtype, suffix) != labels[enumeration_level - 1]:
                self.out.append(
                    "\n\\renewcommand{{\\label{}}}{{{}}}".format(counter_name, label)
                )
        else:
    ...
    def visit_literal_block(self, node):
        ...
        _use_env = _plaintext and not isinstance(
            node.parent, (nodes.footnote, nodes.admonition, nodes.system_message)
        )
        ...
        if literal_env and _use_env and (not _autowidth_table or _use_listings):
            ...
            self.out.append(
                "\\begin{{{}}}{}\n".format(literal_env, self.literal_block_options)
            )
            self.context.append("\n\\end{%s}" % literal_env)
    ...
    def visit_meta(self, node):
        ...
        if name in ("author", "creator", "keywords", "subject", "title"):
            ...
            self.pdfinfo.append("  pdf{}={{{}}},".format(name, content))
        elif name == "producer":
            self.pdfinfo.append("  addtopdfproducer={%s}," % content)
        else:
            # generic interface (case sensitive!)
            self.pdfinfo.append("  pdfinfo={{{}={{{}}}}},".format(name, content))
    ...
    def visit_reference(self, node):
        ...
        if self._reference_label:
            self.out.append(
                "\\{}{{{}}}}}".format(self._reference_label, href.replace("#", ""))
            )
            raise nodes.SkipNode
    ...
    def visit_title(self, node):
        ...
        # Section title
            ...
            if "system-messages" in node.parent["classes"]:
                ...
                self.out.append(r"\{}[{}]{{\color{{red}}".format(section_name, section_title))
            else:
