...
class HTMLTranslator(nodes.NodeVisitor):
    ...
    def starttag(self, node, tagname, suffix="\n", empty=False, **attributes):
        ...
        for name, value in attlist:
            ...
            if isinstance(value, list):
                ...
                parts.append('{}="{}"'.format(name.lower(), self.attval(unicode(value))))
        if empty:
            infix = " /"
        else:
            infix = ""
        ...
        for att_name in "width", "height":
            if att_name in atts:
                if re.match(r"^[0-9.]+$", atts[att_name]):
                    ...
                    atts[att_name] += "px"
                style.append("{}: {};".format(att_name, atts[att_name]))
                del atts[att_name]
        if style:
        ...
        if self.settings.embed_images or ("embed" in node):
            ...
            if err_msg:
                self.document.reporter.error(
                    "Cannot embed image {!r}: {}".format(uri, err_msg)
                )
            else:
                ...
                uri = "data:{};base64,{}".format(mimetype, data64)
    ...
    def depart_label(self, node):
        ...
        if self.settings.footnote_backlinks and len(backrefs) > 1:
            backlinks = [
                '<a href="#{}">{}</a>'.format(ref, i) for (i, ref) in enumerate(backrefs, 1)
            ]
            self.body.append(
