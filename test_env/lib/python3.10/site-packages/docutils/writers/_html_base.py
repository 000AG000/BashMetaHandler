...
class HTMLTranslator(nodes.NodeVisitor):
    ...
    def starttag(self, node, tagname, suffix="\n", empty=False, **attributes):
        ...
        for name, value in attlist:
            ...
            if isinstance(value, list):
                ...
                parts.append(
                    '{}="{}"'.format(name.lower(), self.attval(" ".join(values)))
                )
            else:
                parts.append(
                    f'{name.lower()}="{self.attval(unicode(value))}"'
                )
        if empty:
            infix = " /"
        else:
    ...
    def visit_docinfo_item(self, node, name, meta=True):
        ...
        self.body.append(
            f'<dt class="{name}">{self.language.labels[name]}</dt>\n'
        )
        ...
        if self.settings.embed_images or ("embed" in node):
            ...
            if err_msg:
                self.document.reporter.error(
                    f"Cannot embed image {uri!r}: {err_msg}"
                )
            else:
