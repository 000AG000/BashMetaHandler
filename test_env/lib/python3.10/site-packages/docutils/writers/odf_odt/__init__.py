...
class Writer(writers.Writer):
    ...
    def store_embedded_files(self, zfile):
        ...
        for source, destination in embedded_files:
            ...
            try:
                ...
                self.document.reporter.warning("Can't open file {}.".format(source))
    ...
    def create_meta(self):
        ...
        el1.text = "Docutils/rst2odf.py/{}".format(VERSION)
...
class ODFTranslator(nodes.GenericNodeVisitor):
        ...
        if self.settings.odf_config_file:
            ...
            for rststyle, format in parser.items("Formats"):
                if rststyle not in self.used_styles:
                    self.document.reporter.warning(
                        'Style "{}" is not a style used by odtwriter.'.format(rststyle)
                    )
    ...
    def extract_table_styles(self, styles_str):
        ...
        table_styles = {}
        auto_styles = root.find("{{{}}}automatic-styles".format(CNSD["office"]))
    ...
    def add_header_footer(self, root_el):
        ...
        path = "{{{}}}master-styles".format(NAME_SPACE_1)
        ...
        for item in field_iter:
            if item[0] == ODFTranslator.code_field:
                if item[1] not in (
                    ...
                    msg = "bad field spec: %{}%".format(item[1])
    ...
    def generate_labeled_block(self, node, label):
        label = "{}:".format(self.language.labels[label])
    ...
    def default_visit(self, node):
        self.document.reporter.warning("missing visit_{}".format(node.tagname))

    def default_departure(self, node):
        self.document.reporter.warning("missing depart_{}".format(node.tagname))
        ...
        if isinstance(node.parent, docutils.nodes.footnote):
            ...
            if self.settings.create_links:
                ...
                self.append_child(
                    ...
                    attrib={
                        "text:name": "{}".format(self.citation_id),
                    },
                )
            else:
                el.text = "["
    ...
    def visit_image(self, node):
        ...
        if "uri" in node.attributes:
            ...
            if not (source.startswith("http:") or source.startswith("https:")):
                ...
                if not self.check_file_exists(source):
                    self.document.reporter.warning(
                        "Cannot find image file {}.".format(source)
                    )
                    return
        else:
            return
        if source in self.image_dict:
            ...
            destination = "Pictures/1{:08x}{}".format(
                self.image_count,
                filename,
            )
            if source.startswith("http:") or source.startswith("https:"):
                try:
                    ...
                    self.document.reporter.warning(
                        "Can't open image url {}.".format(source)
                    )
                spec = (
                    source,
                    destination,
    ...
    def get_image_scale(self, node):
        if "scale" in node.attributes:
            ...
            try:
                ...
                self.document.reporter.warning(
                    'Invalid scale for image: "{}"'.format(node.attributes["scale"])
                )
            if scale < 1:  # or scale > 100:
                self.document.reporter.warning(
                    "scale out of range ({}), using 1.".format(scale)
                )
                scale = 1
            scale = scale * 0.01
        else:
    ...
    def get_table_style(self, node):
        ...
        if table_name is not None:
            ...
            if table_style is None:
                ...
                self.document.reporter.warning(
                    'Can\'t find table style "{}".  Using default.'.format(table_name)
                )
    ...
    def depart_table(self, node):
        ...
        attribval = "{:.4f}in".format(self.table_width)
    ...
    def generate_admonition(self, node, label, title=None):
        ...
        if title:
            ...
            el1.text = "{}!".format(translated_label.capitalize())
