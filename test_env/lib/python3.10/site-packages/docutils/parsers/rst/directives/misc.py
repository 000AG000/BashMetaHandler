...
class Include(Directive):
    ...
    def run(self):
        ...
        try:
            ...
            raise self.severe(
                'Problem with "{}" directive:\n{}'.format(self.name, ErrorString(error))
            )
        ...
        include_log = [entry for entry in include_log if entry[2] >= self.lineno]
        if (path, clip_options) in [(pth, opt) for (pth, opt, e) in include_log]:
            raise self.warning(
                'circular inclusion in "%s" directive: %s'
                % (
                    self.name,
                    " < ".join([path] + [pth for (pth, opt, e) in include_log[::-1]]),
                )
            )
        # include as input
        ...
        self.state.document.include_log = [
            (pth, opt, e + len(include_lines) + 2) for (pth, opt, e) in include_log
        ]
        return []
