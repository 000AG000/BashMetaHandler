...
class CTypesData:
    ...
    def _newp(cls, init):
        raise TypeError(
            f"expected a pointer or array ctype, got '{cls._get_c_name()}'"
        )
    ...
    def _convert_to_address(self, BClass):
        if BClass is None:
            raise TypeError(
                f"cannot convert {self._get_c_name()!r} to an address"
            )
        else:
            raise TypeError(
...
class CTypesGenericPtr(CTypesData):
    ...
    def _cast_from(cls, source):
        if source is None:
            ...
            raise TypeError(
                f"bad type for cast to {cls!r}: {type(source).__name__!r}"
            )
