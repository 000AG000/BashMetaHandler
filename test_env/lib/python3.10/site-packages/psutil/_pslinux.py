...
class NetConnections:
    ...
    def get_proc_inodes(self, pid):
        ...
        for fd in os.listdir("{}/{}/fd".format(self._procfs_path, pid)):
            try:
                inode = readlink("{}/{}/fd/{}".format(self._procfs_path, pid, fd))
            ...
            for line in f:
                ...
                try:
                    ...
                    if " " not in line:
                        ...
                        continue
                    raise RuntimeError(
                        "error while parsing {}; malformed line {!r}".format(file, line)
                    )
    ...
    def retrieve(self, kind, pid=None):
        ...
        for proto_name, family, type_ in self.tmap[kind]:
            path = "{}/net/{}".format(self._procfs_path, proto_name)
...
class RootFsDeviceFinder:
    ...
    def ask_sys_dev_block(self):
        path = "/sys/dev/block/{}:{}/uevent".format(self.major, self.minor)
    ...
    def ask_sys_class_block(self):
        needle = "{}:{}".format(self.major, self.minor)
...
def pid_exists(pid):
    ...
    if not _psposix.pid_exists(pid):
        ...
        try:
            ...
            # 'return pid in pids()'
            path = "{}/{}/status".format(get_procfs_path(), pid)
            with open_binary(path) as f:
                for line in f:
...
def ppid_map():
    ...
    for pid in pids():
        try:
            with open_binary("{}/{}/stat".format(procfs_path, pid)) as f:
                data = f.read()
        except (FileNotFoundError, ProcessLookupError):
...
def wrap_exceptions(fun):
    ...
    def wrapper(self, *args, **kwargs):
        try:
            ...
            self._raise_if_zombie()
            if not os.path.exists("{}/{}".format(self._procfs_path, self.pid)):
                raise NoSuchProcess(self.pid, self._name)
            raise
...
class Process:
    ...
    def _raise_if_not_alive(self):
        ...
        os.stat("{}/{}".format(self._procfs_path, self.pid))
    ...
    def _read_status_file(self):
        ...
        in use.
        """
        with open_binary("{}/{}/status".format(self._procfs_path, self.pid)) as f:
            return f.read()

    @wrap_exceptions
    @memoize_when_activated
    def _read_smaps_file(self):
        with open_binary("{}/{}/smaps".format(self._procfs_path, self.pid)) as f:
            return f.read().strip()
    ...
    def exe(self):
        try:
            return readlink("{}/{}/exe".format(self._procfs_path, self.pid))
        except (FileNotFoundError, ProcessLookupError):
            self._raise_if_zombie()
            # no such file error; might be raised also if the
            # path actually exists for system processes with
            # low pids (about 0-20)
            if os.path.lexists("{}/{}".format(self._procfs_path, self.pid)):
                return ""
            raise

    @wrap_exceptions
    def cmdline(self):
        with open_text("{}/{}/cmdline".format(self._procfs_path, self.pid)) as f:
            data = f.read()
        if not data:
    ...
    @wrap_exceptions
    def environ(self):
        with open_text("{}/{}/environ".format(self._procfs_path, self.pid)) as f:
            data = f.read()
    ...
    if os.path.exists("/proc/%s/io" % os.getpid()):
        ...
        def io_counters(self):
            fname = "{}/{}/io".format(self._procfs_path, self.pid)
            fields = {}
    ...
    def memory_info(self):
        ...
        #  ============================================================
        with open_binary("{}/{}/statm".format(self._procfs_path, self.pid)) as f:
    ...
    @wrap_exceptions
    def cwd(self):
        return readlink("{}/{}/cwd".format(self._procfs_path, self.pid))
    ...
    @wrap_exceptions
    def threads(self):
        thread_ids = os.listdir("{}/{}/task".format(self._procfs_path, self.pid))
        thread_ids.sort()
        retlist = []
        hit_enoent = False
    ...
    def open_files(self):
        retlist = []
        files = os.listdir("{}/{}/fd".format(self._procfs_path, self.pid))
        hit_enoent = False
        for fd in files:
            file = "{}/{}/fd/{}".format(self._procfs_path, self.pid, fd)
            try:
                path = readlink(file)
            except (FileNotFoundError, ProcessLookupError):
                # ENOENT == file which is gone in the meantime
    ...
    @wrap_exceptions
    def num_fds(self):
        return len(os.listdir("{}/{}/fd".format(self._procfs_path, self.pid)))
