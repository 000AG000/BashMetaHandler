    ...
    for partition in partitions:
        ...
        if not all:
            ...
            try:
                ...
                debug("skipping {!r}: {}".format(mountpoint, err))
                continue
...
class Process:
    ...
    def _proc_basic_info(self):
        if self.pid == 0 and not os.path.exists(
            "{}/{}/psinfo".format(self._procfs_path, self.pid)
        ):
    ...
    def exe(self):
        try:
            return os.readlink("{}/{}/path/a.out".format(self._procfs_path, self.pid))
        except OSError:
    ...
    def cwd(self):
        ...
        procfs_path = self._procfs_path
        try:
            return os.readlink("{}/{}/path/cwd".format(procfs_path, self.pid))
        except FileNotFoundError:
            os.stat("{}/{}".format(procfs_path, self.pid))  # raise NSP or AD
            return ""
    ...
    def memory_maps(self):
        ...
        for item in rawlist:
            ...
            if not name.startswith("["):
                try:
                    name = os.readlink("{}/{}/path/{}".format(procfs_path, self.pid, name))
                except OSError as err:
                    if err.errno == errno.ENOENT:
                        ...
                        name = "{}/{}/path/{}".format(procfs_path, self.pid, name)
                        hit_enoent = True
                    else:
                        raise
