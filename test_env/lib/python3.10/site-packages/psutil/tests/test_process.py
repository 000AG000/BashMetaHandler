...
class TestProcess(PsutilTestCase):
    ...
    def test_cpu_times_2(self):
        ...
        if (max([user_time, utime]) - min([user_time, utime])) > 0.1:
            raise self.fail("expected: {}, found: {}".format(utime, user_time))

        if (max([kernel_time, ktime]) - min([kernel_time, ktime])) > 0.1:
            raise self.fail("expected: {}, found: {}".format(ktime, kernel_time))
        ...
        def assert_raises_nsp(fun, fun_name):
            try:
                ...
                if WINDOWS and fun_name in ("exe", "name"):
                    return
                raise self.fail(
                    "{!r} didn't raise NSP and returned {!r} instead".format(fun, ret)
                )
