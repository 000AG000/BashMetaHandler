...
class Error(Exception):
    ...
    def __str__(self):
        ...
        if info:
            details = "(%s)" % ", ".join(["{}={!r}".format(k, v) for k, v in info.items()])
        else:
            details = None
    ...
    def __repr__(self):
        ...
        details = ", ".join(["{}={!r}".format(k, v) for k, v in info.items()])
        return "psutil.{}({})".format(self.__class__.__name__, details)
...
def deprecated_method(replacement):
    ...
    def outer(fun):
        msg = "{}() is deprecated and will be removed; use {}() instead".format(
            fun.__name__,
            replacement,
        )
...
def print_color(s, color=None, bold=False, file=sys.stdout):  # pragma: no cover
    ...
    if not term_supports_colors():
        ...
        try:
            ...
            raise ValueError(
                "invalid color {!r}; choose between {!r}".format(color, list(colors.keys()))
            )
        if bold and color <= 7:
            color += 8
    ...
    if PSUTIL_DEBUG:
        ...
        if isinstance(msg, Exception):
            if isinstance(msg, (OSError, IOError, EnvironmentError)):
                ...
                msg = "ignoring %r" % msg
        print(  # noqa
            "psutil-debug [{}:{}]> {}".format(fname, lineno, msg), file=sys.stderr
        )
