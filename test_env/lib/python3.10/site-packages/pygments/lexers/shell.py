...
class BatchLexer(RegexLexer):
    ...
    _label = fr"(?:(?:[^{_nlws}{_punct}+:^]|\^[{_nl}]?[\w\W])*)"
    _label_compound = fr"(?:(?:[^{_nlws}{_punct}+:^)]|\^[{_nl}]?[^)])*)"
    ...
    _core_token = fr'(?:(?:(?:\^[{_nl}]?)?[^"{_nlws}{_punct}])+)'
    _core_token_compound = fr'(?:(?:(?:\^[{_nl}]?)?[^"{_nlws}{_punct})])+)'
    _token = fr"(?:[{_punct}]+|{_core_token})"
    _token_compound = fr"(?:[{_punct}]+|{_core_token_compound})"
    _stoken = fr"(?:[{_punct}]+|(?:{_string}|{_variable}|{_core_token})+)"
