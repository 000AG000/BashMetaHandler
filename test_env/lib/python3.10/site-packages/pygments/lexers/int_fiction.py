...
class Inform6Lexer(RegexLexer):
    ...
    tokens = {
        ...
        "_expression": [
            ...
            (r"(?=sp\b)", Text, "#pop"),
            (
                fr"(?=[{_dquote}{_squote}$0-9#a-zA-Z_])",
                Text,
                ("#pop", "value"),
            ),
        ...
        "string": [
            ...
            (r"\\", String.Escape),
            (
                fr"@(\\\s*[{_newline}]\s*)*@((\\\s*[{_newline}]\s*)*[0-9])*",
                String.Escape,
            ),
            (
...
class Inform7Lexer(RegexLexer):
    ...
    for level in token_variants:
        tokens[level] = {
            ...
            "+documentation": [
                (
                    fr"(?i)({_start})\s*(chapter|example)\s*:[^{_newline}]*",
                    Generic.Heading,
                ),
                (
            ...
            "+i6t-not-inline": [
                (fr"({_start})@c( .*?)?([{_newline}]|\Z)", Comment.Preproc),
                (
            ...
            "+p": [
                ...
                (fr"({_start})@p( .*?)?([{_newline}]|\Z)", Generic.Heading),
                (r"@", Comment.Preproc),
            ],
            "+lines": [
                (fr"({_start})@c( .*?)?([{_newline}]|\Z)", Comment.Preproc),
...
class Tads3Lexer(RegexLexer):
    ...
    def _make_attribute_value_state(
        ...
        escaped_quotes = (
            fr"+|{host_char}(?!{host_char}{{2}})" if host_triple else r""
        )
        return [
        ...
        for index, token, value in RegexLexer.get_tokens_unprocessed(
            ...
            if if_false_level == 0:  # Not in a false #if
                if token is Comment.Preproc and re.match(
                    fr"{pp}if{self._ws_pp}+(0|nil){self._ws_pp}*$\n?", value
                ):
                    if_false_level = 1
            else:  # In a false #if
