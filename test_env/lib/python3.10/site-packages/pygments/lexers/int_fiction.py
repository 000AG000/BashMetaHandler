...
class Inform6Lexer(RegexLexer):
    ...
    tokens = {
        ...
        "_expression": [
            include("_whitespace"),
            (r"(?=sp\b)", Text, "#pop"),
            (r"(?=[{}{}$0-9#a-zA-Z_])".format(_dquote, _squote), Text, ("#pop", "value")),
        ...
        "string": [
            ...
            (r"\\", String.Escape),
            (
                r"@(\\\s*[{}]\s*)*@((\\\s*[{}]\s*)*[0-9])*".format(_newline, _newline),
                String.Escape,
            ),
            (
        ...
        "miscellaneous-keyword?": [
            ...
            (r"{}(?=(\s+|(![^{}]*))*\))".format(_name, _newline), Name.Function, "#pop"),
            default("#pop"),
        ],
    ...
    for level in token_variants:
        tokens[level] = {
            ...
            "+main": [
                ...
                (
                    r"({}|(?<=[\s;:.{}]))\|\s|[{}]{{2,}}".format(_start, _dquote, _newline),
                    Text,
                    "+heading?",
                ),
            ...
            "+documentation": [
                (
                    r"(?i)({})\s*(chapter|example)\s*:[^{}]*".format(_start, _newline),
                    Generic.Heading,
                ),
                (
                ...
                (r"(({})\t.*?[{}])+".format(_start, _newline), using(this, state="+main")),
                (r"[^{}\[]+|[{}\[]".format(_newline, _newline), Text),
                (r"\[", Comment.Multiline, "+comment"),
            ],
            "+i6t-not-inline": [
                (r"({})@c( .*?)?([{}]|\Z)".format(_start, _newline), Comment.Preproc),
                (
                    r"({})@([{}]+|Purpose:)[^{}]*".format(_start, _dash, _newline),
                    Comment.Preproc,
                ),
                (r"({})@p( .*?)?([{}]|\Z)".format(_start, _newline), Generic.Heading, "+p"),
            ],
            "+i6t-use-option": [
            ...
            "+p": [
                ...
                (r"({})@p( .*?)?([{}]|\Z)".format(_start, _newline), Generic.Heading),
                (r"@", Comment.Preproc),
            ],
            "+lines": [
                (r"({})@c( .*?)?([{}]|\Z)".format(_start, _newline), Comment.Preproc),
...
class Tads3Lexer(RegexLexer):
    ...
    def _make_string_state(triple, double, verbatim=None, _escape=_escape):
        if verbatim:
            verbatim = "".join(
                [
                    "(?:{}|{})".format(re.escape(c.lower()), re.escape(c.upper()))
                    for c in verbatim
                ]
            )
    ...
    def _make_attribute_value_state(
        ...
        escaped_quotes = (
            r"+|{}(?!{}{{2}})".format(host_char, host_char) if host_triple else r""
        )
        return [
        ...
        for index, token, value in RegexLexer.get_tokens_unprocessed(
            ...
            if if_false_level == 0:  # Not in a false #if
                if token is Comment.Preproc and re.match(
                    r"{}if{}+(0|nil){}*$\n?".format(pp, self._ws_pp, self._ws_pp), value
                ):
                    if_false_level = 1
            else:  # In a false #if
