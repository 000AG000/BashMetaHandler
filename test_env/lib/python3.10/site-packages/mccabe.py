...
class PathGraph:
    ...
    def to_dot(self):
        ...
        for node, nexts in self.nodes.items():
            for next in nexts:
                print("{} -- {};".format(node.dot_id(), next.dot_id()))
        print("}")
...
class PathGraphingAstVisitor(ASTVisitor):
    ...
    def visitFunctionDef(self, node):
        if self.classname:
            entity = "{}{}".format(self.classname, node.name)
        else:
            entity = node.name
        ...
        if self.graph is not None:
            ...
            self.tail = pathnode
            self.dispatch_list(node.body)
            self.graphs["{}{}".format(self.classname, node.name)] = self.graph
            self.reset()
    ...
    def _subgraph(self, node, name, extra_blocks=()):
        ...
        if self.graph is None:
            ...
            self.graphs["{}{}".format(self.classname, name)] = self.graph
            self.reset()
        else:
            pathnode = self.appendPathNode(name)
            self._subgraph_parse(node, pathnode, extra_blocks)
