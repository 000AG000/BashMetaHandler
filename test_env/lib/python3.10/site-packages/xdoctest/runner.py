...
def gather_doctests(doctest_identifiers, style="auto", analysis="auto", verbose=None):
...
def doctest_module(
    ...
    _debug(f"exclude = {exclude!r}")
    _debug(f"argv = {argv!r}")
    _debug(f"command = {command!r}")
    _debug(f"module_identifier = {module_identifier!r}")
    _debug(f"durations = {durations!r}")
    _debug(f"config = {config!r}")
    _debug(f"verbose = {verbose!r}")
    _debug(f"style = {style!r}")
    _debug("------+ /DEBUG +------")
    ...
    _log(f"Start doctest_module({parsable_identifier!r})", level=2)
    ...
    with warnings.catch_warnings(record=True) as parse_warnlist:
        examples = list(
            core.parse_doctestables(
                parsable_identifier, exclude=exclude, style=style, analysis=analysis
            )
        )
    ...
    if command == "list":
        ...
        if command == "dump":
            ...
            run_summary = _run_examples(enabled_examples, verbose, config, _log=_log)
    ...
    for example in enabled_examples:
        ...
        for part in example._parts:
            ...
            body_part = part.format_part(
                linenos=False, want=False, prefix=False, colored=False, partnos=False
            )
            if part.want:
        ...
        try:
            ...
            if undefined:
                ...
                header_lines.append(
                    "from {} import {}".format(example.modname, ", ".join(undefined))
                )
                body = "\n".join(docstr_lines + header_lines + body_lines)
        except Exception:
...
def _print_summary_report(
    ...
    if parse_warnlist:
        cprint(
            f"\n=== Found {len(parse_warnlist)} parse-time warnings ===",
            "yellow",
        )

        for warn_idx, warn in enumerate(parse_warnlist, start=1):
            cprint(
                f"--- Parse Warning: {warn_idx} / {len(parse_warnlist)} ---",
                "yellow",
            )
            _log(
                utils.indent(
    ...
    if warned:
        cprint(f"\n=== Found {len(warned)} run-time warnings ===", "yellow")
        for warn_idx, example in enumerate(warned, start=1):
            cprint(
                f"--- Runtime Warning: {warn_idx} / {len(warned)} ---",
                "yellow",
            )
            _log(f"example = {example!r}")
    ...
    if failed and len(enabled_examples) > 1:
        ...
        cprint(f"\n=== Found {len(failed)} errors ===", "red")
        for fail_idx, example in enumerate(failed, start=1):
            cprint(f"--- Error: {fail_idx} / {len(failed)} ---", "red")
    ...
    parts = [f"{n} {t}" for n, t in pairs if n > 0]
    ...
    if durations is not None:
        ...
        for example, n_secs in test_time_tups:
            _log(f"time: {n_secs:0.8f}, test: {example.cmdline}")
...
def _update_argparse_cli(add_argument, prefix=None):
    ...
    add_argument(
        *("--time",), dest="time", action="store_true", help=("Same as if durations=0")
    )
    ...
    for alias, kw in add_argument_kws:
        ...
        alias = [
            a.replace("--", "--" + p + "-") if p else a for a in alias for p in prefix
        ]
        if prefix[0]:
