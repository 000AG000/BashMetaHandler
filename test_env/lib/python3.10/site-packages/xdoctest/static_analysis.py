...
from xdoctest.utils.util_import import _platform_pylib_exts  # NOQA; NOQA
from xdoctest.utils.util_import import is_modname_importable
from xdoctest.utils.util_import import modname_to_modpath
from xdoctest.utils.util_import import modpath_to_modname
from xdoctest.utils.util_import import split_modpath
...
class CallDefNode:
    ...
    def __init__(self, callname, lineno, docstr, doclineno, doclineno_end, args=None):
        """
        Args:
            callname (str):
...
class TopLevelVisitor(ast.NodeVisitor):
    ...
    def visit_Module(self, node):
        ...
        if docstr:
            ...
            callname = "__doc__"
            calldef = CallDefNode(callname, doclineno, docstr, doclineno, doclineno_end)
            self.calldefs[callname] = calldef
    ...
    if source is None:  # pragma: no branch
        try:
            ...
            try:
                ...
                print(f"Unable to read fpath = {fpath!r}")
                raise
    try:
        self = TopLevelVisitor.parse(source)
        ...
        if fpath:
            print("Failed to parse docstring for fpath={!r}".format(fpath))
        else:
...
def parse_static_value(key, source=None, fpath=None):
    ...
    if visitor.value is sentinel:
        raise NameError(f"No static variable named {key!r}")
...
def _strip_hashtag_comments_and_newlines(source):
    ...
    def strip_consecutive_newlines(tokens):
        ...
        for token_info in tokens:
            ...
            yield typ, tok, (start_row - skipped_rows, start_col), (
                end_row - skipped_rows,
                end_col,
            ), line
