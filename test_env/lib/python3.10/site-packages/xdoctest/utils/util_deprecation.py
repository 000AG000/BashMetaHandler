...
def schedule_deprecation(
    modname, name="?", type="?", migration="", deprecate=None, error=None, remove=None
):
    """
    ...
    if deprecate is not None:
        ...
        deprecate_str = f" in {deprecate}"
    remove_str = ""
    if remove is not None:
        remove = parse_version(remove)
        remove_str = f" in {remove}"
    error_str = ""
    if error is not None:
        error = parse_version(error)
        error_str = f" in {error}"
    if deprecate is None or current >= deprecate:
        msg = (
            f'The "{name}" {type} was deprecated{deprecate_str}, will cause '
            f"an error{error_str} and will be removed{remove_str}. The current "
            f"version is {current}. {migration}"
        ).strip()
