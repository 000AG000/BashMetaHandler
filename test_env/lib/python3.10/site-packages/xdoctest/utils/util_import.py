...
def is_modname_importable(modname, sys_path=None, exclude=None):
    ...
    Example:
        ...
        False
    """
    modpath = _syspath_modname_to_modpath(modname, sys_path=sys_path, exclude=exclude)
...
def _pkgutil_modname_to_modpath(modname):  # nocover
    ...
    if loader is None:
        raise Exception(f"No module named {modname} in the PYTHONPATH")
...
def _custom_import_modpath(modpath, index=-1):
    ...
    try:
        ...
        msg_parts.append(f"Caused by: {repr(ex)}")
        raise RuntimeError("\n".join(msg_parts))
    return module
    ...
    if not os.path.exists(modpath):
        ...
        if len(parts) > 2:
            ...
            if os.path.exists(archivepath):
                ...
                return module
        raise OSError(f"modpath={modpath} does not exist")
    else:
...
def _static_parse(varname, fpath):
    ...
    if not exists(fpath):
        raise ValueError(f"fpath={fpath!r} does not exist")
...
def modpath_to_modname(
    ...
    if check and relativeto is None:
        if not exists(modpath):
            raise ValueError(f"modpath={modpath} does not exist")
...
def split_modpath(modpath, check=True):
    ...
    if check:
        if not exists(modpath_):
            if not exists(modpath):
                raise ValueError(f"modpath={modpath} does not exist")
            raise ValueError(f"modpath={modpath} is not a module")
