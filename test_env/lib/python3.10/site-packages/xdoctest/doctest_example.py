...
class DocTest:
    ...
    def __init__(
        ...
        if modpath is None:
            ...
            if fpath is not None:
                if fpath != modpath:
                    raise AssertionError("only specify fpath for non-python files")
            self.fpath = modpath
    ...
    def __nice__(self):
        ...
        parts.append(f"{self.callname}:{self.num}")
    ...
    def __repr__(self):
        ...
        return f"<{classname}({devnice}) at {hex(id(self))}>"
    ...
    def format_parts(
        self, linenos=True, colored=None, want=True, offset_linenos=None, prefix=True
    ):
        """
        ...
        if self.module is None:
            if not self.modname.startswith("<"):
                ...
                if global_state.DEBUG_DOCTEST:
                    print(f"Pre-importing modpath = {self.modpath}")
                try:
                    ...
                    if global_state.DEBUG_DOCTEST:
                        ...
                        print(f"Failed to pre-import modpath = {self.modpath}")
                    msg_parts = [
                        (
                    ...
                    if global_state.DEBUG_DOCTEST:
                        print(f"Pre import success: self.module={self.module}")
        ...
        with warnings.catch_warnings(record=True) as self.warn_list:
            for partx, part in enumerate(self._parts):
                ...
                if not did_pre_import:
                    ...
                    if DEBUG:
                        print(f"Global names = {sorted(test_globals.keys())}")
                ...
                try:
                    ...
                    try:
                        ...
                        if part.want:
                            ...
                            exc_got = traceback.format_exception_only(*exception[:2])[
                                -1
                            ]
                            want = part.want
                ...
                # Handle anything that could go wrong
                    ...
                    if verbose > 0:
                        print(f"Test gracefully exists on: ex={ex}")
                    break
                    ...
                    if DEBUG:
                        ...
                        print(f"found_lineno = {found_lineno!r}", file=sys.stderr)
                    ...
                    if found_lineno is None:
                        if DEBUG:
                            ...
                            sys.exit(1)
                        raise ValueError(f"Could not clean traceback: ex = {_ex_dbg!r}")
                    else:
    ...
    def _pre_run(self, verbose):
        if verbose >= 1:
            ...
            if self.block_type == "zero-arg":
                ...
                print(f"* ZERO-ARG FUNC : {self.node}")
            else:
                print(
                    f"* DOCTEST : {self.node}, line {self.lineno}"
    ...
    def _print_captured(self):
        ...
        try:
            ...
            print(f"type(out_text) = {type(out_text)!r}")
            print(f"out_text = {out_text!r}")
    ...
    def _post_run(self, verbose):
        ...
        if self.exc_info is None:
            if verbose >= 1:
                ...
                print(f"* {success}: {self.node}")
        else:
            if verbose >= 1:
                failure = self._color("FAILURE", "red")
                print(f"* {failure}: {self.node}")
