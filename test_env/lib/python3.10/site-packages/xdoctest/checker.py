...
def check_got_vs_want(want, got_stdout, got_eval=constants.NOT_EVALED, runstate=None):
    """
...
def _ellipsis_match(got, want):
    ...
    ws = re.split(rf"\s*{re.escape(ELLIPSIS_MARKER)}\s*", want, flags=re.MULTILINE)
    assert len(ws) >= 2
