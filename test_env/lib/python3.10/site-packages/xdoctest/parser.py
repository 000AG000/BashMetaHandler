...
class DoctestParser:
    ...
    def parse(self, string, info=None):
        ...
        if not isinstance(string, str):
            raise TypeError(f"Expected string but got {string!r}")
        ...
        try:
            ...
            if global_state.DEBUG_PARSER:
                ...
                print(f"info = {ub.repr2(info)}")
                print("-----")
                print(f"orig_ex = {orig_ex}")
                print(f"labeled_lines = {ub.repr2(labeled_lines)}")
                print(f"grouped_lines = {ub.repr2(grouped_lines, nl=3)}")
                print(f"all_parts = {ub.repr2(all_parts)}")
                print("</FAILPOINT>")
                # sys.exit(1)
            raise exceptions.DoctestParseError(
                f"Failed to parse doctest in {failpoint}",
                string=string,
                info=info,
                orig_ex=orig_ex,
            )
    ...
    def _package_groups(self, grouped_lines):
        if global_state.DEBUG_PARSER > 1:
            ...
            print(f"grouped_lines = {ub.repr2(grouped_lines, nl=2)}")
        lineno = 0
        for chunk in grouped_lines:
            if isinstance(chunk, tuple):
                slines, wlines = chunk
                yield from self._package_chunk(slines, wlines, lineno)
    ...
    def _package_chunk(self, raw_source_lines, raw_want_lines, lineno=0):
        ...
        if global_state.DEBUG_PARSER > 1:
            print(f"example.compile_mode = {example.compile_mode!r}")
            print("<YIELD CHUNK>")
        yield example
    ...
    def _group_labeled_lines(self, labeled_lines):
        ...
        if global_state.DEBUG_PARSER > 4:
            print(f"labeled_lines = {labeled_lines!r}")
    ...
    def _locate_ps1_linenos(self, source_lines):
        ...
        def _hack_comment_statements(lines):
            ...
            def balanced_intervals(lines):
                ...
                while b > 0:
                    ...
                    while (
                        not static.is_balanced_statement(lines[a:b], only_tokens=True)
                        and a >= 0
                    ):
                        a -= 1
                    if a < 0:
        ...
        if NEED_16806_WORKAROUND:  # pragma: nobranch
            ps1_linenos = self._workaround_16806(ps1_linenos, exec_source_lines)
    ...
    def _workaround_16806(ps1_linenos, exec_source_lines):
        ...
        for a in ps1_linenos[::-1]:
            ...
            while not static.is_balanced_statement(
                exec_source_lines[a:b], only_tokens=True
            ):
    ...
    def _label_docsrc_lines(self, string):
        ...
        for line_idx, line in line_iter:
            ...
            if global_state.DEBUG_PARSER:  # nocover
                print(f"Next line {line_idx}: {line}")
                print(f"state_indent = {state_indent!r}")
                print(f"match = {match!r}")
                print(f"line_indent = {line_indent!r}")
            ...
            if prev_state == TEXT:
                ...
                raise AssertionError(f"Unknown state prev_state={prev_state}")
                ...
                try:
                    ...
                    for part, norm_line in _complete_source(
                        ...
                        if global_state.DEBUG_PARSER > 4:  # nocover
                            ...
                            print(f"part = {part!r}")
                            print(f"norm_line = {norm_line!r}")
                            print(f"curr_state = {curr_state!r}")
                        if _hasprefix(norm_line, ("...",)):
                            curr_state = DCNT
        ...
        if global_state.DEBUG_PARSER > 1:  # nocover
            ...
            print(f"labeled_lines = {ub.repr2(labeled_lines, nl=1)}")
            print("</FINISH LABELED LINES>")
...
def _complete_source(line, state_indent, line_iter):
    ...
    prefix = norm_line[:4]
    suffix = norm_line[4:]
    assert prefix.strip() in {">>>", "..."}, f"unexpected prefix: {prefix!r}"
    ...
    try:
        while not static.is_balanced_statement(source_parts, only_tokens=True):
            ...
            if prefix.strip() not in {">>>", "...", ""}:  # nocover
                ...
                if error:
                    if global_state.DEBUG_PARSER:
                        print(" * !!!ERROR!!!")
                        print(f" * source_parts = {source_parts!r}")
        ...
        if global_state.DEBUG_PARSER:
            ...
            print(f"source_parts = {ub.repr2(source_parts, nl=2)}")
            print(
                ub.codeblock(
                    r"""
                from xdoctest import static_analysis as static
