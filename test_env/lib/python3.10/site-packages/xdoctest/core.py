...
def parse_freeform_docstr_examples(
    ...
    def doctest_from_parts(parts, num, curr_offset):
        ...
        nested = [
            p.orig_lines if p.want is None else p.orig_lines + p.want.splitlines()
            for p in parts
        ]
    ...
    if global_state.DEBUG_CORE:  # nocover
        print(f"Parsing docstring for callname={callname} in modpath={modpath}")
...
def parse_docstr_examples(
    ...
    if global_state.DEBUG_CORE:  # nocover
        print(
            "Parsing docstring examples for "
    ...
    #     parser = parse_numpy_docstr_examples
        ...
        raise KeyError(f"Unknown style={style}. Valid styles are {DOCTEST_STYLES}")
    ...
    try:
        ...
        if isinstance(ex, exceptions.DoctestParseError):
            ...
            msg += f"{ex.string}\n"
            msg += f"Original Error: {repr(ex.orig_ex)}\n"
        ...
        if isinstance(ex, exceptions.MalformedDocstr):
            ...
            raise
    if global_state.DEBUG_CORE:  # nocover
        print(f"Finished parsing {n_parsed} examples")
...
def _rectify_to_modpath(modpath_or_name):
    ...
    if modpath is None:
        if exists(modpath_or_name):
            ...
            raise ValueError(f"Cannot find module={modpath_or_name}")
    return modpath
...
def package_calldefs(
    ...
    """
    if global_state.DEBUG_CORE:  # nocover
        print(f"Find package calldefs: pkg_identifier = {pkg_identifier!r}")
    ...
    if isinstance(pkg_identifier, types.ModuleType):
        ...
        identifiers = list(
            static_analysis.package_modpaths(pkgpath, with_pkg=True, with_libs=True)
        )
...
def parse_calldefs(module_identifier, analysis="auto"):
    ...
    if global_state.DEBUG_CORE:  # nocover
        print(f"About to parse calldefs with do_dynamic={do_dynamic}")
