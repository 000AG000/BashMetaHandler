    ...
    def update(self, directives):
        ...
        for directive in directives:
            for effect in directive.effects():
                ...
                if key not in self._global_state:
                    warnings.warn(f"Unknown state: {key}")
...
class Directive(utils.NiceRepr):
    ...
    def extract(cls, text):
        ...
        inline = not all(line.strip().startswith("#") for line in text.splitlines())
        #
    ...
    def __nice__(self):
        ...
        if self.args:
            ...
            return f"{prefix}{self.name}({argstr})"
        else:
...
def _is_requires_satisfied(arg, argv=None, environ=None):
    ...
    if arg.startswith("-"):
        ...
        if len(expr_parts) == 1:
            ...
            raise ValueError(f"Too many expr_parts={expr_parts}")
...
def parse_directive_optstr(optpart, inline=None):
    ...
    if name not in COMMANDS:
        msg = f"Unknown directive: {optpart!r}"
        warnings.warn(msg)
    else:
