...
def split_google_docblocks(docstr):
    ...
    if not isinstance(docstr, str):
        raise TypeError(f"Input docstr must be a string. Got {type(docstr)} instead")
...
def parse_google_args(docstr):
    ...
    for key, block in blocks:
        lines = block[0]
        if key == "Args":
            yield from parse_google_argblock(lines)
...
def parse_google_returns(docstr, return_annot=None):
    ...
    for key, block in blocks:
        lines = block[0]
        if key == "Returns":
            yield from parse_google_retblock(lines, return_annot)
        if key == "Yields":
            yield from parse_google_retblock(lines, return_annot)
