...
class AppleHelpBuilder(StandaloneHTMLBuilder):
    ...
    def init(self) -> None:
        ...
        if self.config.applehelp_bundle_id is None:
            raise SphinxError(
                __(
                    "You must set applehelp_bundle_id before "
                    "building Apple Help output"
                )
            )

        self.bundle_path = path.join(
            self.outdir, self.config.applehelp_bundle_name + ".help"  # type: ignore
        )
...
def setup(app: Sphinx) -> Dict[str, Any]:
    ...
    app.add_config_value(
        ...
        lambda self: "{}-{}".format(make_filename(self.project), self.release),
        "applehelp",
    )
