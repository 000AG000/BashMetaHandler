...
def _search_until(tokens: list[Token], idx: int, arg: ast.expr) -> int:
    while idx < len(tokens) and not (
        tokens[idx].line == arg.lineno
        and tokens[idx].utf8_byte_offset == arg.col_offset
    ):
        idx += 1
    return idx
...
def victims(
    ...
    while brace_stack:
        ...
        if token == "," and len(brace_stack) == arg_depth and first_comma_index is None:
            first_comma_index = i
...
def replace_call(
    ...
    *,
    ...
    end_rest = end - 1
    while tokens[end_rest - 1].name == "OP" and tokens[end_rest - 1].src == ",":
        end_rest -= 1
