...
def visit_Assign(
    ...
    if (
        ...
        and node.targets[0].id == node.value.args[0].s
        and not has_starargs(node.value)
    ):
        if (
            is_name_attr(
                node.value.func,
            ...
            and all(
                ...
                and len(tup.elts) == 2
                and isinstance(tup.elts[0], ast.Str)
                and tup.elts[0].s.isidentifier()
            ...
            and not any(keyword.arg == "total" for keyword in node.value.keywords)
        ):
            ...
            and (
                ...
                or (
                    ...
                    and node.value.keywords[0].arg == "total"
                    and isinstance(
            ...
            and node.value.args[1].keys
            and all(
                isinstance(k, ast.Str) and k.s.isidentifier() and k.s not in KEYWORDS
