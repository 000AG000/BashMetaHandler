...
def _supported_version(state: State) -> bool:
    return state.in_annotation and (
        state.settings.min_version >= (3, 10)
        or (
            not state.settings.keep_runtime_typing
            and "annotations" in state.from_imports["__future__"]
        )
    )


def _any_arg_is_str(node_slice: ast.expr) -> bool:
    return isinstance(node_slice, ast.Str) or (
        isinstance(node_slice, ast.Tuple)
        and any(isinstance(elt, ast.Str) for elt in node_slice.elts)
    )
...
def visit_Subscript(
    ...
    if "annotations" not in state.from_imports["__future__"]:
        if (sys.version_info >= (3, 9) and _any_arg_is_str(node.slice)) or (
            sys.version_info < (3, 9)
            and isinstance(node.slice, ast.Index)
            and _any_arg_is_str(node.slice.value)
        ):
            return
