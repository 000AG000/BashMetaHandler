...
def _for_version(
    ...
    *,
    keep_mock: bool,
) -> tuple[Mapping[str, set[str]], Mapping[tuple[str, str], str], Mapping[str, str],]:
...
class FromImport(NamedTuple):
    ...
    def parse(cls, i: int, tokens: list[Token]) -> FromImport:
        ...
        names = [j for j in range(import_token + 1, end) if tokens[j].name == "NAME"]
        ends_by_offset = {}
...
def visit_ImportFrom(
    ...
    if len(removal_idxs) == len(node.names):
        ...
        len(exact_moves) == len(node.names)
        and len({mod for _, mod, _ in exact_moves}) == 1
    ):
