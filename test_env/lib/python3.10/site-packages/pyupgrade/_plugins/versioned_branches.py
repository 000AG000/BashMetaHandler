...
def _eq(test: ast.Compare, n: int) -> bool:
    return (
        ...
        and isinstance(test.comparators[0], ast.Num)
        and test.comparators[0].n == n
    )
...
def _compare_to_3(
    ...
    if not (
        isinstance(test.ops[0], op)
        and isinstance(test.comparators[0], ast.Tuple)
        and len(test.comparators[0].elts) >= 1
...
def visit_If(
    ...
    if (
        ...
        (
            ...
            and (
                _eq(node.test, 2)
                or _compare_to_3(node.test, ast.Lt, min_version[1])
                or any(
        ...
        (
            ...
            and (
                _eq(node.test, 3)
                or _compare_to_3(node.test, (ast.Gt, ast.GtE))
                or _compare_to_3(node.test, ast.GtE, min_version[1])
                or any(
