...
def _eligible(keywords: list[ast.keyword]) -> bool:
    if len(keywords) == 1:
        ...
        return (
            _is_literal_kwarg(keywords[0], "maxsize", None)
            and _is_literal_kwarg(keywords[1], "typed", False)
        ) or (
            _is_literal_kwarg(keywords[1], "maxsize", None)
            and _is_literal_kwarg(keywords[0], "typed", False)
        )
    else:
        return False
...
def visit_Call(
    ...
    if (
        state.settings.min_version >= (3, 8)
        and not node.args
