...
def visit_Call(
    ...
    if (
        state.settings.min_version >= (3, 6)
        ...
        and isinstance(node.keywords[0].value.func, ast.Name)
        and node.keywords[0].value.func.id == "locals"
        and len(node.keywords[0].value.args) == 0
        and len(node.keywords[0].value.keywords) == 0
    ):
