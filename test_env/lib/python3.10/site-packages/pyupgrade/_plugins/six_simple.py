...
def visit_Attribute(
    ...
    if (
        ...
        if isinstance(parent, ast.Call) and is_a_native_literal_call(
            parent, state.from_imports
        ):
            return
        ...
        and node.value.value.id == "six"
        and node.value.attr == "moves"
        and node.attr == "xrange"
    ):
